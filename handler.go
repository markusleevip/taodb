...
func Get(s *Server) Handler {
	return HandlerFunc(func(w resp.ResponseWriter, c *resp.Command) {
		switch c.ArgN() {
		case 1:
			value,err :=(*s.db).Get(string(c.Arg(0)))
			if err!=nil{
...
func Set(s *Server) Handler{
	return HandlerFunc(func(w resp.ResponseWriter, c *resp.Command) {
		switch c.ArgN() {
		case 2:
			err :=(*s.db).Set(string(c.Arg(0)),c.Arg(1).Bytes())
			if err!=nil{
				w.AppendError(UnknownCommand(c.Name+" "+string(c.Arg(0))))
			}else {
				w.AppendBulk([]byte("ok"))
			}
		default:
